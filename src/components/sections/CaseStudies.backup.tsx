...
export default function CaseStudies() {
  ...
  return (
    <section id="casos" className="bg-[#FAFAF7] py-28 overflow-hidden">
      <div className="max-w-6xl mx-auto px-6">
        <RevealOnScroll direction="right" className="mb-14 flex items-end justify-between gap-6">
          <div>
            <p className="text-[12px] font-inter uppercase tracking-[0.15em] text-[#0F766E] mb-4">Resultados reales</p>
            <h2 className="text-[clamp(2rem,4vw,2.8rem)] font-inter font-semibold leading-tight tracking-[-0.02em]">
              ...
              <span className="text-[#0F766E]">trabaja por ti</span>
            </h2>
          </div>
          <div className="hidden md:flex gap-3 flex-shrink-0">
            <button onClick={prev} aria-label="Anterior"
              className="w-10 h-10 rounded-full border border-[#1A1A1A]/15 hover:border-[#0F766E] hover:text-[#0F766E] flex items-center justify-center transition-colors cursor-pointer">
              <ChevronLeft size={17}/>
            </button>
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex gap-4" style={{touchAction:'pan-y'}}>
            {cases.map((c, i) => (
              <div key={i} className="flex-none w-[min(320px,80vw)]">
          ...
          {cases.map((_, i) => (
            <button key={i} onClick={() => emblaApi?.scrollTo(i)} aria-label={`Caso ${i+1}`}
              className={`h-1.5 rounded-full transition-all duration-300 cursor-pointer ${
                i === selected ? 'w-6 bg-[#0F766E]' : 'w-1.5 bg-[#1A1A1A]/20'
              }`}/>
          ))}
        </div>
      </div>
